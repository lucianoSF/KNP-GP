...
df = pd.read_csv('../saidas/GP/' + code + '_solution.csv', sep=';')



y = df['Constructed']
y_after_ls = df['AfterLS']
y_best = df['Best']
...
ax.plot(x, y, "-", label="Constructed Solution", linewidth=2.0, markersize=12, color='#006bb3')
ax.plot(x, y_best, "-", label="Best Solution", linewidth=2.0, markersize=12, color='#c7053d')
ax.plot(x, y_after_ls, ".", label="After LS", color='#cca300')
